...
function sampev.onServerMessage(clr, msg)
    ...
    if (work or work1) and msg:find('[Ошибка] {FFFFFF}Вы еще ничего не заработали и не можете получить зарплату!') then
        lua_thread.create(function() 
            BeginToPoint(-92.8096,93.2234,3.1172, 5, -255, true)  
            work = false
            work1 = false
...
function imgui.OnDrawFrame()
    ...
    if main_windows_state.v then
        ...
        if vkladki[1] == true then -- Настройки бота
			...
			imgui.SameLine(210)
			imgui.Text(u8'Настройки бота') imgui.NewLine()
        ...
        if vkladki[3] == true then -- Увед вк
            ...
            imgui.NewLine() imgui.NewLine()
			imgui.SameLine(210)
			imgui.Text(u8'Уведомления ВК') imgui.NewLine()
        ...
        if vkladki[4] == true then -- Настройки 
			...
			imgui.NewLine() imgui.NewLine()
			imgui.SameLine(200)
            imgui.Text(u8'Настройки') imgui.NewLine()
            imgui.Separator() imgui.NewLine() imgui.NewLine()
            ...
            imgui.NewLine() imgui.NewLine()
					imgui.SameLine(15) if imgui.Button(u8'Проверить обновления') then
					...
					downloadUrlToFile('https://raw.githubusercontent.com/danil8515/Farm-bot/master/version.json', fpath, function(id, status, p1, p2) -- ссылку на ваш гитхаб где есть строчки которые я ввёл в теме или любой другой сайт
						...
						if info and info.latest then
							...
							new = 1
										sampAddChatMessage(('[Farm Bot]: {FFFFFF}Доступно обновление!'), 0xF1CB09)
								else -- если меньше, то
							update = false -- не даём обновиться
							sampAddChatMessage(('[Farm Bot]: {FFFFFF}У вас установлена последния версия!'), 0xF1CB09)
							end
						end
						end
					end
        ...
        if vkladki[5] == true then -- Информация
			...
			imgui.NewLine() imgui.NewLine()
			imgui.SameLine(200)
            imgui.Text(u8'Информация') imgui.NewLine()
...
function goupdate()
    gou = false
    sampAddChatMessage(('[Farm Bot]: Обнаружено обновление. AutoReload может конфликтовать. Обновляюсь...'), 0xF1CB09)
    sampAddChatMessage(('[Farm Bot]: Текущая версия: '..thisScript().version..". Новая версия: "..ver), 0xF1CB09)
    wait(300)
    downloadUrlToFile(updatelink, thisScript().path, function(id3, status1, p13, p23) -- качает ваш файлик с latest version
        if status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
        sampAddChatMessage(('[Farm Bot]: Обновление завершено!'), 0xF1CB09)
        thisScript():reload()
        end
    end)
end
